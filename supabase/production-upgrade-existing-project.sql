...
-- ---------- Checkpoint A/B/C: fixed class challenge packages ----------
...
-- ---------- 2026-07-18 CP39 sentence read-aloud and public privacy ----------
ALTER TABLE daily_attempts ADD COLUMN IF NOT EXISTS sentence_speech_success_count INT;
ALTER TABLE daily_attempts ADD COLUMN IF NOT EXISTS sentence_speech_bonus INT;
ALTER TABLE challenge_sessions ADD COLUMN IF NOT EXISTS mask_scoreboard_names BOOLEAN DEFAULT TRUE NOT NULL;
...
BEGIN
  IF NEW.practice = false THEN
    INSERT INTO activity_feed (student_name, school, grade, score, message)
    VALUES (
      mask_student_name(NEW.student_name), NEW.school, NEW.grade, NEW.score,
      '完成了 12 題挑戰，獲得了 ' || NEW.score || ' 分！'
    );
  END IF;
  RETURN NEW;
